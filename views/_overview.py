...
def show_overall(df_stats, df_classified):
    ...
    selected_cat = st.selectbox("카테고리 선택", categories, key="overall_cat")
    period = st.radio("기간 선택", ["1년 전체", "계절별"], horizontal=True, key="overall_period")

    if period == "계절별":
        season = st.selectbox("계절 선택", ["봄", "여름", "가을", "겨울"], key="overall_season")
...
def show_school(df_schools, df_classified):
    ...
    with col2:
        ...
        school_name = st.selectbox("부대 선택", sorted(filtered_schools["school_name"].dropna().tolist()), key="school_name")
    ...
    categories = ["전체"] + sorted(df_classified["category"].dropna().unique().tolist())
    selected_cat = st.selectbox("카테고리 선택", categories, key="school_cat")
    period = st.radio("기간 선택", ["1년 전체", "계절별"], horizontal=True, key="school_period")

    if period == "계절별":
        season = st.selectbox("계절 선택", ["봄", "여름", "가을", "겨울"], key="school_season")
    ...
    if selected_cat != "전체":
        cat_menus = df_classified[df_classified["category"] == selected_cat]["dish_name_raw"].tolist()
        df_filtered_menu = df_filtered[df_filtered["dish_name"].isin(cat_menus)]
    else:
        df_filtered_menu = df_filtered

    st.subheader("가장 많이 나온 메뉴 TOP 20")
    top = df_filtered_menu["dish_name"].value_counts().head(20).reset_index()
    top.columns = ["메뉴명", "등장횟수"]
    fig = px.bar(top, x="등장횟수", y="메뉴명", orientation="h",
                 color="등장횟수", color_continuous_scale="Oranges")
...
def show():
    ...
    if mode == "📊 전체 통계":
        show_overall(df_stats, df_classified)
    else:
        show_school(df_schools, df_classified)
